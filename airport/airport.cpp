...
void WaitingAirplanes::permitEntryToFirstInQueue() {
    ...
    // lock the class ; TODO add better description
...
void Runway::setRunwayIsBlocked(bool status){
    // TODO set Mutex so that only one thread can set the status in parallel: see WaitingAirplanes
    lock_guard<mutex> lock(_mutex);
    _isBlocked = status;
}

bool Runway::isBlocked(){
    // TODO set Mutex so that only one thread can gwt the status in parallel
    lock_guard<mutex> lock(_mutex);
    return _isBlocked;
}
